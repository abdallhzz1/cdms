...
type AcademicYear = {
  ...
  semester1_start:string|null; semester1_end:string|null; semester2_start:string|null;
  semester2_end:string|null; summer_start:string|null; summer_end:string|null;
...
type YearForm = {code:string;start_date:string;end_date:string;semester1_start:string;semester1_end:string;semester2_start:string;semester2_end:string;summer_start:string;summer_end:string;is_current:boolean;status:AcademicYear['status'];notes:string};
...
const emptyYear:YearForm={code:'',start_date:'',end_date:'',semester1_start:'',semester1_end:'',semester2_start:'',semester2_end:'',summer_start:'',summer_end:'',is_current:false,status:'active',notes:''};
...
export function AcademicCalendarPage(){
  ...
  const yearMutation=useMutation({
    mutationFn:()=>apiFetch<AcademicYear>(editingYear?`/academic-years/${editingYear.id}`:'/academic-years',{method:editingYear?'PUT':'POST',body:{...yearForm,semester1_start:yearForm.semester1_start||null,semester1_end:yearForm.semester1_end||null,semester2_start:yearForm.semester2_start||null,semester2_end:yearForm.semester2_end||null,summer_start:yearForm.summer_start||null,summer_end:yearForm.summer_end||null,notes:yearForm.notes||null}}),
  ...
  const openEditYear=()=>{if(!selectedYear)return;setEditingYear(selectedYear);setYearForm({code:selectedYear.code,start_date:selectedYear.start_date,end_date:selectedYear.end_date,semester1_start:selectedYear.semester1_start??'',semester1_end:selectedYear.semester1_end??'',semester2_start:selectedYear.semester2_start??'',semester2_end:selectedYear.semester2_end??'',summer_start:selectedYear.summer_start??'',summer_end:selectedYear.summer_end??'',is_current:selectedYear.is_current,status:selectedYear.status,notes:selectedYear.notes??''});setError('');setYearModal(true);};
  ...
  return <div className="space-y-5">
    ...
    <Modal isOpen={yearModal} onClose={()=>setYearModal(false)} title={editingYear?'تعديل العام الأكاديمي':'إنشاء عام أكاديمي'} maxWidth="2xl"><form className="space-y-4" onSubmit={(e:FormEvent)=>{e.preventDefault();yearMutation.mutate();}}><ModalError text={error}/><div className="grid gap-3 md:grid-cols-3"><Field label="رمز العام"><input required pattern="[0-9]{4}/[0-9]{4}" placeholder="2026/2027" className={inputClass} value={yearForm.code} onChange={e=>setYearForm({...yearForm,code:e.target.value})}/></Field><Field label="بداية العام"><input required type="date" className={inputClass} value={yearForm.start_date} onChange={e=>setYearForm({...yearForm,start_date:e.target.value})}/></Field><Field label="نهاية العام"><input required type="date" className={inputClass} value={yearForm.end_date} onChange={e=>setYearForm({...yearForm,end_date:e.target.value})}/></Field></div><DatePair title="الفصل الأول" start={yearForm.semester1_start} end={yearForm.semester1_end} onStart={value=>setYearForm({...yearForm,semester1_start:value})} onEnd={value=>setYearForm({...yearForm,semester1_end:value})}/><DatePair title="الفصل الثاني" start={yearForm.semester2_start} end={yearForm.semester2_end} onStart={value=>setYearForm({...yearForm,semester2_start:value})} onEnd={value=>setYearForm({...yearForm,semester2_end:value})}/><DatePair title="الفصل الصيفي" start={yearForm.summer_start} end={yearForm.summer_end} onStart={value=>setYearForm({...yearForm,summer_start:value})} onEnd={value=>setYearForm({...yearForm,summer_end:value})}/><div className="grid gap-3 md:grid-cols-2"><Field label="حالة العام"><select className={inputClass} value={yearForm.status} onChange={e=>setYearForm({...yearForm,status:e.target.value as AcademicYear['status']})}><option value="planned">مخطط</option><option value="active">نشط</option><option value="closed">مغلق</option></select></Field><label className="flex items-center gap-2 self-end rounded-xl border border-slate-200 px-3 py-2.5 text-sm font-bold"><input type="checkbox" checked={yearForm.is_current} onChange={e=>setYearForm({...yearForm,is_current:e.target.checked})}/>اعتماد هذا العام كعام حالي</label></div><Field label="ملاحظات"><textarea className={`${inputClass} h-20 py-2`} value={yearForm.notes} onChange={e=>setYearForm({...yearForm,notes:e.target.value})}/></Field><div className="flex justify-end gap-2"><Button type="button" variant="outline" onClick={()=>setYearModal(false)}>إلغاء</Button><Button type="submit" isLoading={yearMutation.isPending}>حفظ العام</Button></div></form></Modal>
...
function DatePair({title,start,end,onStart,onEnd}:{title:string;start:string;end:string;onStart:(value:string)=>void;onEnd:(value:string)=>void}){return <fieldset className="rounded-xl border border-slate-200 p-3"><legend className="px-2 text-xs font-black text-slate-600">{title}</legend><div className="grid gap-3 sm:grid-cols-2"><Field label="البداية"><input type="date" className={inputClass} value={start} onChange={e=>onStart(e.target.value)}/></Field><Field label="النهاية"><input type="date" className={inputClass} value={end} onChange={e=>onEnd(e.target.value)}/></Field></div></fieldset>}
