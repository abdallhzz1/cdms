...
type ClinicalPeriod={id:number;academic_year_id:number;code:string;name_ar:string;name_en:string|null;sequence:number;start_date:string;end_date:string;weeks_count:number;status:'planned'|'active'|'closed';notes:string|null};
type Rotation = {
  ...
  schedule_scope:'period'|'annual'; clinical_period:ClinicalPeriod|null;
  blocks:Array<{id:number;block_code:string;from_week:number;to_week:number}>;
};
type Overview = {academic_year:AcademicYear;periods:ClinicalPeriod[];events:CalendarEvent[];rotations:Rotation[]};
...
const emptyPeriod={code:'P1',name_ar:'الفترة السريرية الأولى',name_en:'Clinical Period 1',sequence:1,start_date:'',end_date:'',weeks_count:12,status:'active' as ClinicalPeriod['status'],notes:''};
...
export function AcademicCalendarPage(){
  ...
  const [editingPeriod,setEditingPeriod]=useState<ClinicalPeriod|null>(null);
  ...
  const periodMutation=useMutation({mutationFn:()=>apiFetch(editingPeriod?`/clinical-periods/${editingPeriod.id}`:'/clinical-periods',{method:editingPeriod?'PUT':'POST',body:{academic_year_id:Number(yearId),...periodForm,notes:periodForm.notes||null}}),onSuccess:async()=>{await refresh();setPeriodModal(false);setNotice(editingPeriod?'تم تحديث الفترة السريرية.':'تمت إضافة الفترة السريرية.');setError('');},onError:e=>setError(errorText(e))});
  const deletePeriod=useMutation({mutationFn:(id:number)=>apiFetch(`/clinical-periods/${id}`,{method:'DELETE'}),onSuccess:async()=>{await refresh();setNotice('تم حذف الفترة السريرية.');},onError:e=>setError(errorText(e))});
  ...
  const openNewPeriod=()=>{const next=(overview?.periods.length??0)+1;const previous=overview?.periods.at(-1);const start=previous?new Date(`${previous.end_date}T12:00:00`):new Date(`${selectedYear?.start_date}T12:00:00`);if(previous)start.setDate(start.getDate()+1);const end=new Date(start);end.setDate(end.getDate()+83);setEditingPeriod(null);setPeriodForm({...emptyPeriod,code:`P${next}`,name_ar:`الفترة السريرية ${next}`,name_en:`Clinical Period ${next}`,sequence:next,start_date:start.toISOString().slice(0,10),end_date:end.toISOString().slice(0,10)});setError('');setPeriodModal(true);};
  const openEditPeriod=(item:ClinicalPeriod)=>{setEditingPeriod(item);setPeriodForm({code:item.code,name_ar:item.name_ar,name_en:item.name_en??'',sequence:item.sequence,start_date:item.start_date,end_date:item.end_date,weeks_count:item.weeks_count,status:item.status,notes:item.notes??''});setError('');setPeriodModal(true);};
  ...
  return <div className="space-y-5">
    <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
      ...
      <div className="flex flex-wrap gap-2"><Button variant="outline" onClick={exportExcel} isLoading={exporting} disabled={!overview}><FileSpreadsheet className="ml-1.5 h-4 w-4"/>تصدير Excel</Button>{canManage&&<><Button variant="outline" onClick={openNewYear}><Plus className="ml-1.5 h-4 w-4"/>عام جديد</Button><Button variant="outline" onClick={openEditYear} disabled={!selectedYear}><Settings2 className="ml-1.5 h-4 w-4"/>إعدادات العام</Button><Button variant="outline" onClick={openNewPeriod} disabled={!selectedYear}><Layers3 className="ml-1.5 h-4 w-4"/>إضافة فترة</Button><Button onClick={openNewEvent} disabled={!selectedYear}><Plus className="ml-1.5 h-4 w-4"/>إضافة حدث</Button></>}</div>
    </div>
    ...
    {years.length===0?<Card className="p-10 text-center"><CalendarDays className="mx-auto h-10 w-10 text-slate-300"/><h2 className="mt-3 font-black">لا يوجد عام أكاديمي</h2><p className="mt-1 text-sm text-slate-500">أنشئ العام وحدد بدايته ونهايته قبل إعداد الدورات والجداول.</p>{canManage&&<Button className="mt-4" onClick={openNewYear}>إنشاء أول عام</Button>}</Card>:<>
      ...
      {overviewQuery.isLoading?<LoadingState/>:overviewQuery.isError?<ErrorState onRetry={()=>overviewQuery.refetch()}/>:overview&&<>
        <section className="grid gap-3 lg:grid-cols-3">{overview.periods.map(period=><article key={period.id} className="rounded-2xl border border-indigo-100 bg-white p-4 shadow-sm"><div className="flex items-start justify-between gap-2"><div><span className="rounded-lg bg-indigo-50 px-2 py-1 text-[10px] font-black text-indigo-700">{period.code} · {period.weeks_count} أسبوع</span><h3 className="mt-2 font-black text-slate-900">{period.name_ar}</h3><p className="mt-1 text-xs text-slate-500">{formatDate(period.start_date)} — {formatDate(period.end_date)}</p></div>{canManage&&<div className="flex"><Button size="sm" variant="ghost" onClick={()=>openEditPeriod(period)}><Edit3 className="h-4 w-4"/></Button><Button size="sm" variant="ghost" className="text-red-600" onClick={()=>{if(confirm(`حذف ${period.name_ar}؟`))deletePeriod.mutate(period.id)}}><Trash2 className="h-4 w-4"/></Button></div>}</div></article>)}</section>
    ...
    <Modal isOpen={periodModal} onClose={()=>setPeriodModal(false)} title={editingPeriod?'تعديل الفترة السريرية':'إضافة فترة سريرية'} maxWidth="xl"><form className="space-y-4" onSubmit={(e:FormEvent)=>{e.preventDefault();periodMutation.mutate();}}><ModalError text={error}/><div className="grid gap-3 sm:grid-cols-2"><Field label="رمز الفترة"><input required className={inputClass} value={periodForm.code} onChange={e=>setPeriodForm({...periodForm,code:e.target.value})}/></Field><Field label="الترتيب"><input required type="number" min="1" max="10" className={inputClass} value={periodForm.sequence} onChange={e=>setPeriodForm({...periodForm,sequence:Number(e.target.value)})}/></Field><Field label="اسم الفترة"><input required className={inputClass} value={periodForm.name_ar} onChange={e=>setPeriodForm({...periodForm,name_ar:e.target.value})}/></Field><Field label="الاسم بالإنجليزية"><input className={inputClass} value={periodForm.name_en} onChange={e=>setPeriodForm({...periodForm,name_en:e.target.value})}/></Field><Field label="تاريخ البداية"><input required type="date" min={selectedYear?.start_date} max={selectedYear?.end_date} className={inputClass} value={periodForm.start_date} onChange={e=>setPeriodForm({...periodForm,start_date:e.target.value})}/></Field><Field label="تاريخ النهاية"><input required type="date" min={periodForm.start_date} max={selectedYear?.end_date} className={inputClass} value={periodForm.end_date} onChange={e=>setPeriodForm({...periodForm,end_date:e.target.value})}/></Field><Field label="عدد الأسابيع"><input required type="number" min="1" max="52" className={inputClass} value={periodForm.weeks_count} onChange={e=>setPeriodForm({...periodForm,weeks_count:Number(e.target.value)})}/></Field><Field label="الحالة"><select className={inputClass} value={periodForm.status} onChange={e=>setPeriodForm({...periodForm,status:e.target.value as ClinicalPeriod['status']})}><option value="planned">مخططة</option><option value="active">فعالة</option><option value="closed">منتهية</option></select></Field></div><Field label="ملاحظات"><textarea className={`${inputClass} h-20 py-2`} value={periodForm.notes} onChange={e=>setPeriodForm({...periodForm,notes:e.target.value})}/></Field><div className="flex justify-end gap-2"><Button type="button" variant="outline" onClick={()=>setPeriodModal(false)}>إلغاء</Button><Button type="submit" isLoading={periodMutation.isPending}>حفظ الفترة</Button></div></form></Modal>
...
function RotationRow({item,detailed=false}:{item:Rotation;detailed?:boolean}){return <div className="grid gap-3 p-4 sm:grid-cols-[130px_1fr_auto] sm:items-center"><div className="text-xs font-bold text-slate-500">{item.start_date?<><p>{formatDate(item.start_date)}</p><p className="mt-1">حتى {formatDate(item.end_date)}</p></>:<span className="text-amber-700">غير مؤرخة</span>}</div><div><div className="flex flex-wrap items-center gap-2"><span className="rounded-full border border-indigo-200 bg-indigo-50 px-2 py-0.5 text-[10px] font-bold text-indigo-700">{item.clinical_period?.name_ar??'جدول سنوي'}</span><h3 className="text-sm font-black">{item.name}</h3><span className="font-mono text-[10px] text-slate-400">{item.code}</span></div><p className="mt-1 text-xs text-slate-500">{levels[item.academic_level]} · {item.duration_weeks??'—'} أسبوع{detailed&&item.blocks.length?` · ${item.blocks.map(block=>`${block.block_code} (أسبوع ${block.from_week}-${block.to_week})`).join('، ')}`:''}</p></div><div className="flex items-center gap-1.5"><GraduationCap className="h-4 w-4 text-teal-600"/><span className="text-[10px] font-bold text-slate-500">{item.distribution_status==='published'?'منشور':item.distribution_status==='approved'?'معتمد':item.distribution_status?'قيد الإعداد':'دون جدول'}</span></div></div>}
