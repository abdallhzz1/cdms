...
const statusLabel: Record<string, string> = { draft: 'مسودة', submitted: 'بانتظار اعتماد العميد', approved: 'معتمد' };
const purposeLabel: Record<string, string> = { annual_performance: 'تقييم أداء سنوي', renewal: 'تجديد التكليف', reappointment: 'إعادة تكليف' };
const recommendationLabel: Record<string, string> = { renew: 'يوصى بالتجديد', renew_with_conditions: 'يوصى بالتجديد بشروط', not_recommend: 'لا يوصى بالتجديد' };

function formatDate(value?: string | null) { return value ? new Intl.DateTimeFormat('ar-PS', { dateStyle: 'medium' }).format(new Date(value)) : '—'; }
...
export function DepartmentHeadEvaluationsPage() {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: async (evaluation) => { setSelectedId(evaluation.id); setSearchParams({ evaluation: String(evaluation.id) }); await invalidate(); setNotice('تم حفظ مسودة التقييم.'); },
  });
  const actionMutation = useMutation({
    ...
    onSuccess: async (evaluation, variables) => { await invalidate(); queryClient.setQueryData(['department-head-evaluation', evaluation.id], evaluation); setNotice(variables.action === 'approve' ? 'تم اعتماد التقييم رسميًا.' : variables.action === 'submit' ? 'تم توقيع التقييم وإرساله للاعتماد.' : 'أعيد التقييم إلى مسودة للمراجعة.'); },
  });
  ...
  const errorText = (error: unknown) => error instanceof ApiError ? error.message : 'تعذر حفظ التقييم. حاول مرة أخرى.';
  ...
  if (optionsQuery.isError || evaluationsQuery.isError) return <ErrorState title="تعذر تحميل تقييمات رؤساء الأقسام" message="تحقق من الصلاحيات أو أعد المحاولة." onRetry={() => { optionsQuery.refetch(); evaluationsQuery.refetch(); }} />;
  ...
  const downloadWord = () => {
    if (!selected) return;
    const html = `<!doctype html><html dir="rtl"><head><meta charset="utf-8"><style>body{font-family:Arial,sans-serif;color:#172033;margin:38px;line-height:1.6}h1{text-align:center;font-size:19px;margin:0 0 6px}h2{font-size:14px;margin:24px 0 8px;color:#0f766e}.sub{text-align:center;color:#526176;font-size:12px;margin-bottom:22px}table{width:100%;border-collapse:collapse;margin:10px 0 18px;font-size:11px}th{background:#e7f7f5}th,td{border:1px solid #b8c8d0;padding:7px;text-align:right;vertical-align:top}.score{font-size:22px;font-weight:bold;color:#0f766e;text-align:center;padding:12px;background:#f0fdfa}.sign{margin-top:35px;display:flex;gap:45px}.sign div{width:45%;border-top:1px solid #64748b;padding-top:7px;font-size:11px}.muted{color:#64748b}</style></head><body><h1>جامعة الخليل - كلية الطب البشري</h1><h1>نموذج تقييم أداء رئيس القسم وإعادة التكليف</h1><p class="sub">Annual Performance Evaluation & Renewal Assessment</p><table><tr><th>العام الأكاديمي</th><td>${escapeHtml(selected.academic_year_name || '')}</td><th>القسم</th><td>${escapeHtml(selected.department_name || '')}</td></tr><tr><th>رئيس القسم</th><td>${escapeHtml(selected.department_head_name || '')}</td><th>الغرض</th><td>${escapeHtml(purposeLabel[selected.evaluation_purpose] || '')}</td></tr></table><p class="muted">سلم التقدير: 5 ممتاز | 4 جيد جدًا | 3 جيد | 2 مقبول | 1 غير مرضٍ. النتيجة الموزونة = (الدرجة ÷ 5) × وزن المحور.</p><table><tr><th>المحور</th><th>الوزن</th><th>الدرجة</th><th>الموزون</th><th>الملاحظات</th></tr>${selected.domains.map(domain => `<tr><td>${escapeHtml(domain.name_ar)}</td><td>${domain.weight}%</td><td>${domain.score} / 5</td><td>${domain.weighted_score || ((domain.score / 5) * domain.weight).toFixed(1)} / ${domain.weight}</td><td>${escapeHtml(domain.comment || '—')}</td></tr>`).join('')}<tr><th colspan="3">المجموع الكلي</th><th colspan="2">${selected.overall_score} / 100 - ${escapeHtml(selected.overall_rating || '')}</th></tr></table><h2>أهم الإنجازات خلال العام الأكاديمي</h2><ul>${selected.major_achievements.map(item => `<li>${escapeHtml(item)}</li>`).join('')}</ul><h2>مجالات التطوير المستقبلية</h2><ul>${selected.development_areas.map(item => `<li>${escapeHtml(item)}</li>`).join('')}</ul><h2>التوصية بشأن التجديد</h2><p><strong>${escapeHtml(recommendationLabel[selected.recommendation || ''] || '—')}</strong></p><p>${escapeHtml(selected.recommendation_notes || '')}</p><div class="sign"><div>المقيّم: ${escapeHtml(selected.evaluator_name || '____________________')}<br>${escapeHtml(selected.evaluator_role || '')}<br>التوقيع والتاريخ: ${formatDate(selected.evaluator_signed_at)}</div><div>العميد: ${escapeHtml(selected.dean_name || '____________________')}<br>${escapeHtml(selected.dean_role || '')}<br>التوقيع والتاريخ: ${formatDate(selected.dean_signed_at)}</div></div></body></html>`;
  ...
  return <div className="mx-auto max-w-7xl space-y-5 pb-12">
    ...
    <PageHeader title="تقييم رؤساء الأقسام وإعادة التكليف" description="نموذج رسمي موزون وموقّع، مبني على معايير تقييم القيادة الأكاديمية والتجديد السنوي.">
      {canCreate && <Button onClick={startNew}><Plus className="ml-2 h-4 w-4" />تقييم جديد</Button>}
      {selected && canExport && <><Button variant="outline" onClick={downloadWord}><FileDown className="ml-2 h-4 w-4" />تصدير Word</Button><Button variant="outline" onClick={printEvaluation}><Printer className="ml-2 h-4 w-4" />طباعة / حفظ PDF</Button></>}
    </PageHeader>
    {notice && <div className="flex items-center justify-between rounded-2xl border border-teal-100 bg-teal-50 px-4 py-3 text-sm font-bold text-teal-800"><span>{notice}</span><button onClick={() => setNotice('')} className="text-xs underline">إخفاء</button></div>}

    <div className="grid gap-5 lg:grid-cols-[19rem_1fr]">
      <Card className="h-fit rounded-3xl border-slate-200 p-4"><div className="mb-3 flex items-center gap-2"><History className="h-4 w-4 text-teal-600" /><h2 className="text-sm font-black text-slate-800">السجل الرسمي</h2></div><div className="space-y-2">{(evaluationsQuery.data || []).length ? (evaluationsQuery.data || []).map(evaluation => <button key={evaluation.id} onClick={() => { setSelectedId(evaluation.id); setSearchParams({ evaluation: String(evaluation.id) }); }} className={`w-full rounded-2xl border p-3 text-right transition ${selectedId === evaluation.id ? 'border-teal-300 bg-teal-50' : 'border-slate-200 bg-white hover:border-teal-200'}`}><div className="flex items-start justify-between gap-2"><p className="text-xs font-black text-slate-800">{evaluation.department_head_name}</p><span className={`rounded-lg px-2 py-1 text-[10px] font-bold ${evaluation.status === 'approved' ? 'bg-teal-100 text-teal-800' : evaluation.status === 'submitted' ? 'bg-amber-50 text-amber-700' : 'bg-slate-100 text-slate-600'}`}>{statusLabel[evaluation.status]}</span></div><p className="mt-1 text-[11px] text-slate-500">{evaluation.department_name} · {evaluation.academic_year_name}</p><p className="mt-2 text-xs font-bold text-teal-700">{evaluation.overall_score} / 100 · {evaluation.overall_rating || 'غير مكتمل'}</p></button>) : <p className="rounded-2xl border border-dashed border-slate-200 p-5 text-center text-xs text-slate-500">لا توجد تقييمات موثقة بعد.</p>}</div></Card>
      ...
      <Card className="rounded-3xl border-slate-200 p-4 sm:p-6">
        {!canCreate && !selected ? <div className="py-16 text-center"><ShieldCheck className="mx-auto h-10 w-10 text-teal-600" /><h2 className="mt-3 font-black text-slate-800">اختر تقييمًا من السجل</h2><p className="mt-1 text-sm text-slate-500">لديك صلاحية الاطلاع والتصدير فقط.</p></div> : <>
          <div className="mb-5 flex flex-col gap-3 border-b border-slate-100 pb-4 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="font-black text-slate-800">{selected ? 'التقييم الرسمي' : 'مسودة تقييم جديدة'}</h2><p className="mt-1 text-xs text-slate-500">المجموع يُحتسب آليًا من الدرجات والأوزان ولا يمكن إدخاله يدويًا.</p></div>{selected && <span className={`w-fit rounded-xl px-3 py-1.5 text-xs font-bold ${selected.status === 'approved' ? 'bg-teal-100 text-teal-800' : selected.status === 'submitted' ? 'bg-amber-50 text-amber-700' : 'bg-slate-100 text-slate-600'}`}>{statusLabel[selected.status]}</span>}</div>
