...
import { Award, BookOpen, BriefcaseBusiness, Building2, Camera, CheckCircle2, Download, FileText, GraduationCap, KeyRound, Mail, MapPin, Pencil, Plus, Save, ShieldCheck, Trash2, UserRound } from 'lucide-react';
...
function Detail({ label, value, ltr = false }: { label: string; value?: string | number | null; ltr?: boolean }) {
  return <div className="rounded-2xl border border-slate-100 bg-slate-50/70 p-4"><dt className="text-xs font-semibold text-slate-500">{label}</dt><dd dir={ltr ? 'ltr' : undefined} className={`mt-1.5 min-h-5 text-sm font-bold text-slate-800 ${ltr ? 'text-start' : ''}`}>{value || '—'}</dd></div>;
}
...
export function MyProfilePage() {
  ...
  const missingLabels: Record<string, string> = { name: tr('الاسم بالعربية', 'Arabic name'), full_name_en: tr('الاسم بالإنجليزية', 'English name'), phone: tr('رقم الهاتف', 'Phone number'), specialty: tr('التخصص', 'Specialty'), academic_degree: tr('الدرجة الأكاديمية', 'Academic degree'), bio: tr('النبذة المهنية', 'Professional summary') };
  ...
  return <div className="mx-auto max-w-6xl space-y-5 pb-10">
    ...
    <Card className="border border-slate-200 shadow-sm">
      <div className="h-20 bg-gradient-to-l from-teal-800 via-teal-700 to-slate-800 sm:h-24" />
      <div className="px-5 pb-5 sm:px-7"><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div className="flex min-w-0 items-end gap-4"><div className="relative -mt-10 grid h-24 w-24 shrink-0 place-items-center overflow-hidden rounded-3xl border-4 border-white bg-teal-50 text-2xl font-black text-teal-700 shadow-md">{profile.avatar_url ? <img src={profile.avatar_url} alt={profile.name} className="h-full w-full object-cover" /> : profile.name.slice(0, 1)}<label className="absolute inset-x-1 bottom-1 flex h-8 cursor-pointer items-center justify-center rounded-xl bg-slate-900/70 text-white hover:bg-slate-900" title={tr('تحديث الصورة', 'Update photo')}><Camera className="h-4 w-4" /><input type="file" accept="image/jpeg,image/png,image/webp" className="hidden" onChange={onPhotoSelect} disabled={avatarMutation.isPending} /></label></div><div className="min-w-0 pb-1"><h1 className="truncate text-xl font-black text-slate-900 sm:text-2xl">{profile.name}</h1>{profile.full_name_en && <p dir="ltr" className="mt-1 truncate text-start text-sm text-slate-500">{profile.full_name_en}</p>}<div className="mt-2 flex flex-wrap gap-2">{profile.roles.map(role => <span key={role} className="rounded-lg bg-teal-50 px-2.5 py-1 text-xs font-bold text-teal-800">{roleLabel(role, locale)}</span>)}</div></div></div><Button variant="outline" onClick={() => setProfileModalOpen(true)}><Pencil className="me-2 h-4 w-4" />{tr('تعديل البيانات', 'Edit details')}</Button></div>
        <div className="mt-5 grid gap-3 border-t border-slate-100 pt-4 sm:grid-cols-[1fr_auto] sm:items-center"><div><div className="flex items-center justify-between text-xs font-bold"><span className="text-slate-600">{tr('اكتمال الملف', 'Profile completion')}</span><span className="text-teal-700">{profile.completion_percent}%</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-teal-600 transition-all" style={{ width: `${profile.completion_percent}%` }} /></div>{profile.missing_fields.length > 0 && <p className="mt-2 text-xs text-slate-500">{tr('البيانات المتبقية:', 'Still needed:')} {profile.missing_fields.map(key => missingLabels[key] || key).join('، ')}</p>}</div><div className="flex items-center gap-2 text-sm text-slate-600"><Mail className="h-4 w-4 text-teal-600" /><span dir="ltr">{profile.email}</span></div></div>
      </div>
    </Card>

    <nav className="flex gap-2 overflow-x-auto rounded-2xl border border-slate-200 bg-white p-2 shadow-sm" aria-label={tr('أقسام الملف الشخصي', 'Profile sections')}>{sections.map(section => <button key={section.id} onClick={() => selectSection(section.id)} className={`flex min-w-max flex-1 items-center justify-center gap-2 rounded-xl px-4 py-2.5 text-sm font-bold transition ${activeSection === section.id ? 'bg-teal-700 text-white shadow-sm' : 'text-slate-600 hover:bg-slate-50 hover:text-teal-700'}`}><section.icon className="h-4 w-4" />{section.label}{section.count !== undefined && <span className={`rounded-md px-1.5 py-0.5 text-xs ${activeSection === section.id ? 'bg-white/20' : 'bg-slate-100'}`}>{section.count}</span>}</button>)}</nav>

    {activeSection === 'overview' && <div className="grid gap-5 lg:grid-cols-[1.35fr_1fr]">
      <Card className="border border-slate-200 p-5 sm:p-6"><div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-xl bg-teal-50 text-teal-700"><UserRound className="h-5 w-5" /></span><div><h2 className="text-base font-black text-slate-900">{tr('المعلومات الأساسية', 'Basic information')}</h2><p className="mt-1 text-sm text-slate-500">{tr('بيانات الاتصال والهوية المستخدمة في النظام.', 'Identity and contact details used across the system.')}</p></div></div><dl className="mt-5 grid gap-3 sm:grid-cols-2"><Detail label={tr('الاسم بالعربية', 'Arabic name')} value={profile.name} /><Detail label={tr('الاسم بالإنجليزية', 'English name')} value={profile.full_name_en} ltr /><Detail label={tr('رقم الهاتف', 'Phone number')} value={profile.phone} ltr /><Detail label={tr('الرقم الوظيفي', 'Staff code')} value={profile.staff_code} ltr /></dl></Card>
      <Card className="border border-slate-200 p-5"><h2 className="flex items-center gap-2 text-base font-black text-slate-900"><Building2 className="h-5 w-5 text-teal-600" />{tr('الارتباط الأكاديمي', 'Academic affiliation')}</h2><div className="mt-4 space-y-3 text-sm">{displayDepartment && <p className="flex items-center gap-2 font-semibold text-slate-700"><GraduationCap className="h-4 w-4 text-teal-600" />{displayDepartment}</p>}{displaySite && <p className="flex items-center gap-2 font-semibold text-slate-700"><MapPin className="h-4 w-4 text-teal-600" />{displaySite}</p>}{profile.training_sites.map(site => <p key={site.id} className="flex items-center gap-2 text-slate-700"><MapPin className="h-4 w-4 text-teal-600" />{ar ? site.name_ar : (site.name_en || site.name_ar)}{site.is_primary && <span className="rounded-md bg-teal-50 px-2 py-1 text-xs font-bold text-teal-700">{tr('أساسي', 'Primary')}</span>}</p>)}{profile.assigned_levels.length > 0 && <div className="flex flex-wrap gap-2">{profile.assigned_levels.map(level => <span key={level} className="rounded-lg bg-slate-100 px-2.5 py-1.5 text-xs font-bold text-slate-700">{ar ? `دفعة ${level === 'fourth' ? 'رابعة' : level === 'fifth' ? 'خامسة' : 'سادسة'}` : `${level} cohort`}</span>)}</div>}{!displayDepartment && !displaySite && !profile.training_sites.length && !profile.assigned_levels.length && <p className="text-slate-500">{tr('لا توجد ارتباطات مسندة.', 'No affiliations assigned.')}</p>}</div></Card>
      {profile.employment && <Card className="border border-slate-200 p-5 sm:p-6 lg:col-span-2"><div className="flex items-center justify-between gap-3"><h2 className="flex items-center gap-2 text-base font-black text-slate-900"><BriefcaseBusiness className="h-5 w-5 text-teal-600" />{tr('المعلومات الوظيفية', 'Employment information')}</h2><span className="rounded-lg bg-slate-100 px-2.5 py-1 text-xs font-bold text-slate-600">{tr('للعرض فقط', 'Read only')}</span></div><dl className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3"><Detail label={tr('التخصص', 'Specialty')} value={profile.specialty} /><Detail label={tr('الدرجة الأكاديمية', 'Academic degree')} value={profile.academic_degree} /><Detail label={tr('رقم الترخيص', 'License number')} value={profile.employment.license_number} ltr /><Detail label={tr('نوع العقد', 'Contract type')} value={profile.employment.contract_type} /><Detail label={tr('بداية العقد', 'Contract start')} value={profile.employment.contract_start} /><Detail label={tr('نهاية العقد', 'Contract end')} value={profile.employment.contract_end} /></dl><p className="mt-4 text-xs leading-5 text-slate-500">{tr('تُحدّث هذه البيانات من إدارة الكادر لضمان اعتماد المعلومات الوظيفية.', 'These fields are managed by staff administration to keep employment information verified.')}</p></Card>}
    </div>}
