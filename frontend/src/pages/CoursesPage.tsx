...
import { 
  ...
  Stethoscope, UploadCloud, Download, X,
...
const getLevelName = (level: string, locale: string) => {
  const norm = normalizeLevel(level);
  if (norm === 'fourth') return locale === 'ar' ? 'السنة الرابعة' : '4th Year';
  if (norm === 'fifth') return locale === 'ar' ? 'السنة الخامسة' : '5th Year';
  return locale === 'ar' ? 'السنة السادسة' : '6th Year';
};
...
export function CoursesPage() {
  ...
  const currentTotalHours = filteredCourses.reduce((acc, c) => acc + (c.credit_hours || 0), 0);
  ...
  const handleDownloadTemplate = () => {
    const csvContent = "\uFEFF" + 
      "code,name_ar,name_en,credit_hours,academic_level,is_active,description\n" +
      "MEDI401,مساق الطب الباطني العام,General Internal Medicine,6,fourth,1,تدريب سريري سنوي في الطب الباطني للسنة الرابعة\n" +
      "SURG402,مساق الجراحة العامة,General Surgery,6,fourth,1,تدريب سريري سنوي في الجراحة العامة للسنة الرابعة\n" +
      "PEDI501,مساق طب الأطفال,Pediatrics,4,fifth,1,تدريب سريري سنوي في طب الأطفال للسنة الخامسة\n" +
      "OBGY502,مساق النسائية والتوليد,Obstetrics and Gynecology,4,fifth,1,تدريب سريري سنوي في النسائية والتوليد للسنة الخامسة\n" +
      "PSYC601,مساق الطب النفسي السريري,Clinical Psychiatry,3,sixth,1,تدريب سريري سنوي في الطب النفسي للسنة السادسة\n";
  ...
  return (
    <div className="space-y-4 pb-16 px-1 sm:px-0">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] sm:text-[11px] text-slate-500 mt-0.5">
              {locale === 'ar' ? 'إدارة دليل المساقات السريرية المعتمدة (السنة الرابعة، الخامسة، والسادسة).' : 'Annual curriculum management for 4th, 5th, and 6th clinical years.'}
            </p>
          </div>
        </div>
                      ...
                      <TableBody>
                        {importRows.slice(0, 10).map((row, idx) => (
                          <TableRow key={idx}>
                            ...
                            <TableCell className="text-xs">{getLevelName(row.academic_level, locale)}</TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
                    </Table>
      ...
      {/* Main Single Table Layout for Annual System */}
      <div className="bg-white rounded-2xl border border-slate-200 shadow-2xs overflow-hidden">
        <div className="p-3 bg-slate-50 border-b border-slate-200 font-bold text-xs text-slate-800 flex justify-between items-center">
          <span>
            {selectedLevel === 'all' 
              ? (locale === 'ar' ? 'كافة المساقات السريرية المسجلة' : 'All Registered Clinical Courses')
              : (locale === 'ar' ? `مساقات ${getLevelName(selectedLevel, 'ar')}` : `Courses for ${getLevelName(selectedLevel, 'en')}`)}
          </span>
          <span className="text-teal-700 font-semibold">
            {filteredCourses.length} {locale === 'ar' ? 'مساقاً' : 'Courses'} ({currentTotalHours} {locale === 'ar' ? 'ساعة معتمدة' : 'Credits'})
          </span>
        </div>
        ...
        {filteredCourses.length > 0 ? (
            ...
            <table className={`w-full border-collapse text-xs min-w-[600px] ${locale === 'ar' ? 'text-right' : 'text-left'}`}>
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50/50 text-slate-500 font-semibold text-[11px]">
                  ...
                  <th className="p-2.5 text-center w-28">{locale === 'ar' ? 'رقم المساق' : 'Code'}</th>
                  <th className={`p-2.5 ${locale === 'ar' ? 'text-right' : 'text-left'}`}>{locale === 'ar' ? 'اسم المساق' : 'Course Name'}</th>
                  <th className="p-2.5 text-center w-28">{locale === 'ar' ? 'السنة السريرية' : 'Clinical Level'}</th>
                  <th className="p-2.5 text-center w-24">{locale === 'ar' ? 'الساعات' : 'Credits'}</th>
                ...
                {filteredCourses.map((course, idx) => (
                  <tr key={course.id} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="p-2.5 text-center text-slate-600 font-medium">
                      {getLevelName(course.academic_level, locale)}
                    </td>
          ...
          <div className="p-8 text-center text-slate-400 text-xs space-y-1">
            <p className="font-semibold">{locale === 'ar' ? 'لا توجد مساقات مضافة لهذه الفئة' : 'No courses found for this category'}</p>
            <p className="text-[11px] text-slate-400">{locale === 'ar' ? 'يمكنك استخدام زر "استيراد CSV" أو "إضافة مساق سريري" لإضافة المساقات' : 'Use Import CSV or Add Course to add courses'}</p>
          </div>
        )}
      </div>
