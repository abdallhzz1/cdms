...
export function PublicClinicalAttendancePage() {
  ...
  const [student, setStudent] = useState<{name:string;university_number:string}|null>(null); const [message, setMessage] = useState(''); const [error, setError] = useState(''); const [loading, setLoading] = useState(false);
  ...
  useEffect(() => {
    ...
    void apiFetch<{attendance_intent:string}>('/public/clinical-attendance/begin',{method:'POST',body:{qr_token:qr}})
      .then(result => {
        setIntent(result.attendance_intent);
        void clinicalAttendanceIdentity().then(identity => {
          setStudent(identity.student);
          void submit();
        }).catch(() => undefined);
      })
      .catch(exception => setError(exception instanceof Error ? exception.message : 'انتهت صلاحية الرمز. امسح رمز المشرف الحالي مرة أخرى.'));
  }, [qr, submit]);
  ...
  return <main dir="rtl" className="min-h-screen bg-slate-50 p-4 text-slate-900"><div className="mx-auto max-w-md space-y-5 py-5"><Link to="/portal/clinical-schedule" className="text-xs font-bold text-teal-700">← العودة للجدول السريري</Link><section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><div className="mb-5 flex items-center gap-3"><span className="grid h-11 w-11 place-items-center rounded-2xl bg-teal-50 text-teal-700"><ShieldCheck/></span><div><h1 className="font-black">تسجيل الحضور السريري</h1><p className="mt-1 text-xs text-slate-500">تم فتح تسجيل الحضور من رمز المشرف.</p></div></div>{!student ? <div className="space-y-3">{!challenge ? <><label className="block text-xs font-bold">الرقم الجامعي<input dir="ltr" value={number} onChange={event => setNumber(event.target.value.replace(/\D/g,''))} className="mt-1.5 h-11 w-full rounded-xl border border-slate-200 px-3 text-right outline-none focus:border-teal-500" placeholder="مثال: 22130000"/></label><button disabled={loading || !number || (!!qr && !intent)} onClick={request} className="h-11 w-full rounded-xl bg-teal-700 text-sm font-black text-white disabled:opacity-50">{qr && !intent ? 'جارٍ تجهيز الحضور...' : 'إرسال رمز التحقق'}</button></> : <><label className="block text-xs font-bold">رمز التحقق من البريد الجامعي<input dir="ltr" value={otp} onChange={event => setOtp(event.target.value.replace(/\D/g,'').slice(0,6))} className="mt-1.5 h-11 w-full rounded-xl border border-slate-200 px-3 text-center tracking-[0.45em] outline-none focus:border-teal-500" inputMode="numeric"/></label><p className="rounded-xl bg-teal-50 p-3 text-xs text-teal-800">بعد التحقق سيتم حفظ هذا الهاتف لمدة 30 يوماً وتسجيل حضورك تلقائياً.</p><button disabled={loading || otp.length !== 6} onClick={verify} className="h-11 w-full rounded-xl bg-teal-700 text-sm font-black text-white disabled:opacity-50">تحقق وسجل الحضور</button></>}</div> : <><div className="mb-4 rounded-2xl bg-teal-50 p-3 text-xs text-teal-900"><b>{student.name}</b><span dir="ltr" className="mr-2">{student.university_number}</span>{qr && !message && <p className="mt-2">جارٍ تسجيل الحضور...</p>}</div>{!qr && <LiveQrScanner onDecoded={decoded} onError={scannerError}/>}</>}{message && <p className="mt-4 flex gap-2 rounded-xl bg-emerald-50 p-3 text-xs font-bold text-emerald-800"><CheckCircle2 className="h-4 w-4 shrink-0"/>{message}</p>}{error && <p className="mt-4 rounded-xl bg-red-50 p-3 text-xs font-bold text-red-700">{error}</p>}</section><p className="px-3 text-center text-[11px] leading-5 text-slate-500">الرمز مؤقت ويتغير تلقائياً. لا تشارك الشاشة أو الرابط مع أي شخص.</p></div></main>;
}
