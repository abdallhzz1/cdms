  ...
  return <main dir="rtl" className="min-h-screen bg-slate-50 px-3 py-4 text-slate-800 sm:px-6 sm:py-8">
    <div className="mx-auto max-w-4xl space-y-3 sm:space-y-5">
      ...
      {data&&<section className="space-y-3 sm:space-y-4">
        ...
        {data.schedule.length===0?<Card className="rounded-2xl border border-dashed border-slate-300 p-6 text-center"><CalendarDays className="mx-auto h-8 w-8 text-slate-300"/><h3 className="mt-2 text-sm font-black">لا يوجد جدول منشور لك حالياً</h3><p className="mt-1 text-[11px] leading-5 text-slate-500">يرجى المراجعة لاحقاً أو التواصل مع إدارة الدائرة السريرية.</p></Card>:<div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="hidden grid-cols-[110px_1fr_1fr] border-b border-slate-200 bg-slate-50 px-4 py-2 text-[11px] font-black text-slate-500 sm:grid"><span>الأسبوع والفترة</span><span>المساق / النشاط</span><span>مكان الدوام والمشرف</span></div>{data.schedule.map((item,index)=><article key={`${item.block?.block_code}-${index}`} className={`grid gap-2 border-b border-slate-100 p-3 last:border-0 sm:grid-cols-[110px_1fr_1fr] sm:items-center sm:px-4 ${item.item_type==='activity'?'bg-amber-50/60':'bg-white'}`}><div className="flex items-center justify-between sm:block"><div><p className={`text-xs font-black ${item.item_type==='activity'?'text-amber-800':'text-teal-800'}`}>الأسبوع {item.block?.from_week||'—'}</p><p className="mt-0.5 text-[9px] text-slate-400 sm:text-[10px]">{formatDate(item.block?.start_date||null)} — {formatDate(item.block?.end_date||null)}</p></div><span className={`rounded-md px-2 py-1 text-[9px] font-black sm:hidden ${item.item_type==='activity'?'bg-amber-100 text-amber-800':'bg-teal-50 text-teal-700'}`}>{item.block?.block_code||'—'}</span></div><div><p className="text-[9px] font-bold text-slate-400">{item.course?.code||'مساق سريري'} {item.academic_year?`· ${item.academic_year}`:''}</p><p className="mt-0.5 text-xs font-black text-slate-800">{item.item_type==='activity'?(item.activity?.label||'نشاط أكاديمي'):(item.course?.name_ar||item.course?.name_en||'الدورة السريرية')}</p></div>{item.item_type==='activity'?<div className="rounded-lg bg-amber-100/70 px-2.5 py-2 text-[10px] font-bold text-amber-800">لا يوجد دوام سريري في هذا الأسبوع</div>:<div className="grid grid-cols-2 gap-1.5 text-[10px] sm:block sm:space-y-1"><p className="flex items-center gap-1.5 rounded-lg bg-slate-50 px-2 py-1.5 font-bold text-slate-700"><MapPin className="h-3 w-3 shrink-0 text-teal-600"/>{item.training_site?.name_ar||item.training_site?.name||'—'}</p><p className="flex items-center gap-1.5 rounded-lg bg-slate-50 px-2 py-1.5 font-bold text-slate-700"><Stethoscope className="h-3 w-3 shrink-0 text-teal-600"/>{item.supervisor?.full_name_ar||item.supervisor?.name||'شاغر'}</p></div>}</article>)}</div>}
