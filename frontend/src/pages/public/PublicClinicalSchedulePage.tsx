  ...
  return <main dir="rtl" className="min-h-screen bg-slate-50 px-3 py-4 text-slate-800 sm:px-6 sm:py-8">
    <div className="mx-auto max-w-4xl space-y-4 sm:space-y-5">
      ...
      {data&&<section className="space-y-4">
        <Card className="rounded-[28px] border border-slate-200 p-5 sm:p-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-teal-600 text-white"><GraduationCap className="h-6 w-6"/></div><div><h2 className="text-lg font-black">{data.student.name}</h2><p className="mt-1 text-xs text-slate-500"><span className="font-mono">{data.student.university_number}</span> · {levelNames[data.student.academic_level]||data.student.academic_level}</p></div></div><Button variant="outline" className="rounded-xl text-xs" onClick={reset}>استعلام لطالب آخر</Button></div></Card>

        <div className="grid gap-4 md:grid-cols-[.8fr_1.2fr]">
          <Card className="rounded-[28px] border border-slate-200 p-5"><div className="mb-4 flex items-center gap-2"><UsersRound className="h-5 w-5 text-teal-600"/><h2 className="font-black">المجموعة والزملاء</h2></div>{data.subgroup?<><div className="mb-4 rounded-2xl border border-teal-100 bg-teal-50 p-4 text-slate-800"><p className="text-xs text-teal-700">المجموعة الرئيسية</p><p className="mt-1 text-2xl font-black">{data.group?.name||'—'} <span className="text-base font-bold text-teal-600">/ {data.subgroup.name}</span></p></div><div className="space-y-2">{data.members.map((member,index)=><div key={`${member.name}-${index}`} className={`flex items-center gap-3 rounded-xl border p-3 ${member.is_current_student?'border-teal-200 bg-teal-50':'border-slate-100 bg-slate-50'}`}><div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-xs font-black ${member.is_current_student?'bg-teal-600 text-white':'bg-white text-slate-500'}`}>{index+1}</div><span className="text-sm font-bold text-slate-700">{member.name}</span>{member.is_current_student&&<span className="mr-auto rounded-full bg-white px-2 py-1 text-[10px] font-black text-teal-700">أنت</span>}</div>)}</div></>:<p className="rounded-2xl bg-slate-50 p-4 text-xs font-bold leading-6 text-slate-800">لا توجد مجموعة مثبتة لك ضمن الجدول المنشور حالياً.</p>}</Card>

          <div className="space-y-3"><div className="flex items-center gap-2 px-1"><CalendarDays className="h-5 w-5 text-teal-700"/><h2 className="font-black">فترات الدوام المنشورة</h2><span className="mr-auto rounded-full bg-teal-50 px-2.5 py-1 text-xs font-black text-teal-700">{data.schedule.length}</span></div>{data.schedule.length===0?<Card className="rounded-[28px] border border-dashed border-slate-300 p-8 text-center"><CalendarDays className="mx-auto h-10 w-10 text-slate-300"/><h3 className="mt-3 font-black">لا يوجد جدول منشور لك حالياً</h3><p className="mt-2 text-xs leading-6 text-slate-500">قد تكون الدائرة السريرية ما زالت تعمل على التوزيع. يرجى المراجعة لاحقاً أو التواصل مع الإدارة.</p></Card>:data.schedule.map((item,index)=><Card key={`${item.block?.block_code}-${index}`} className={`rounded-[26px] border p-5 ${item.item_type==='activity'?'border-amber-200 bg-amber-50/70':'border-slate-200'}`}><div className="mb-4 flex items-start justify-between gap-3"><div><p className={`text-[11px] font-bold ${item.item_type==='activity'?'text-amber-700':'text-teal-600'}`}>{item.course?.code||'مساق سريري'} · {item.academic_year||''}</p><h3 className="mt-1 text-lg font-black">{item.item_type==='activity'?(item.activity?.label||'نشاط أكاديمي'):(item.course?.name_ar||item.course?.name_en||'الدورة السريرية')}</h3>{item.item_type==='activity'&&<p className="mt-1 text-xs font-bold text-amber-700">لا يوجد توزيع على مستشفى أو مشرف في هذا الأسبوع.</p>}</div><span className={`shrink-0 rounded-xl px-3 py-2 text-xs font-black ${item.item_type==='activity'?'bg-amber-100 text-amber-800':'bg-teal-50 text-teal-700'}`}>{item.block?.block_code||'—'}</span></div>{item.item_type==='activity'?<div className="flex items-start gap-2 rounded-xl bg-white/80 p-3 text-xs"><CalendarDays className="mt-0.5 h-4 w-4 shrink-0 text-amber-600"/><div><p className="text-slate-400">الفترة</p><p className="mt-1 font-black text-slate-700">الأسبوع {item.block?.from_week||'—'} إلى {item.block?.to_week||'—'} · {formatDate(item.block?.start_date||null)} — {formatDate(item.block?.end_date||null)}</p></div></div>:<div className="grid gap-2 text-xs sm:grid-cols-2"><div className="flex items-start gap-2 rounded-xl bg-slate-50 p-3"><MapPin className="mt-0.5 h-4 w-4 shrink-0 text-teal-600"/><div><p className="text-slate-400">المستشفى / الموقع</p><p className="mt-1 font-black text-slate-700">{item.training_site?.name_ar||item.training_site?.name||'—'}</p></div></div><div className="flex items-start gap-2 rounded-xl bg-slate-50 p-3"><Stethoscope className="mt-0.5 h-4 w-4 shrink-0 text-teal-600"/><div><p className="text-slate-400">المشرف السريري</p><p className="mt-1 font-black text-slate-700">{item.supervisor?.full_name_ar||item.supervisor?.name||'شاغر'}</p></div></div><div className="flex items-start gap-2 rounded-xl bg-slate-50 p-3 sm:col-span-2"><Building2 className="mt-0.5 h-4 w-4 shrink-0 text-teal-600"/><div><p className="text-slate-400">الفترة</p><p className="mt-1 font-black text-slate-700">الأسبوع {item.block?.from_week||'—'} إلى {item.block?.to_week||'—'} · {formatDate(item.block?.start_date||null)} — {formatDate(item.block?.end_date||null)}</p></div></div></div>}</Card>)}</div>
        </div>
