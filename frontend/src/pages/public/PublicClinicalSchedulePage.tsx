...
export function PublicClinicalSchedulePage() {
  ...
  const periods=useMemo(()=>Array.from(new Map((data?.schedule??[]).filter(item=>item.clinical_period).map(item=>[item.clinical_period!.id,item.clinical_period!])).values()).sort((a,b)=>a.sequence-b.sequence),[data]);
  const displayedSchedule=useMemo(()=>periodId?(data?.schedule??[]).filter(item=>String(item.clinical_period?.id)===periodId):(data?.schedule??[]),[data,periodId]);
  ...
  useEffect(()=>{if(data&&periods.length&&!periods.some(period=>String(period.id)===periodId))setPeriodId(String(periods[0].id));},[data,periods,periodId]);
  ...
  const reset=()=>{setNumber('');setChallenge('');setOtp('');setEmailHint('');setAccessToken('');setData(null);setPeriodId('');setError('');setMessage('');setOtpSeconds(0)};
  ...
  return <main dir="rtl" className="min-h-screen bg-slate-50 px-3 py-4 text-slate-800 sm:px-6 sm:py-8">
    <div className="mx-auto max-w-4xl space-y-3 sm:space-y-5">
      ...
      {data&&<section className="space-y-3 sm:space-y-4">
        ...
        <div className="flex flex-col gap-2 px-1 sm:flex-row sm:items-center"><div className="flex items-center gap-2"><CalendarDays className="h-4.5 w-4.5 text-teal-700"/><h2 className="text-sm font-black sm:text-base">الجدول الأسبوعي</h2><span className="rounded-full bg-teal-50 px-2 py-0.5 text-[10px] font-black text-teal-700">{displayedSchedule.length} أسابيع</span></div>{periods.length>0&&<label className="sm:mr-auto"><span className="sr-only">اختر الفترة السريرية</span><select value={periodId} onChange={event=>setPeriodId(event.target.value)} className="h-9 w-full rounded-xl border border-slate-200 bg-white px-3 text-xs font-bold outline-none focus:border-teal-500 sm:w-56">{periods.map(period=><option key={period.id} value={period.id}>{period.code} — {period.name_ar}</option>)}</select></label>}</div>
        {displayedSchedule.length===0?<Card className="rounded-2xl border border-dashed border-slate-300 p-6 text-center"><CalendarDays className="mx-auto h-8 w-8 text-slate-300"/><h3 className="mt-2 text-sm font-black">لا يوجد جدول منشور لك في هذه الفترة</h3><p className="mt-1 text-[11px] leading-5 text-slate-500">يرجى المراجعة لاحقاً أو التواصل مع إدارة الدائرة السريرية.</p></Card>:<div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="overflow-x-auto"><table className="w-full min-w-[690px] table-fixed border-collapse text-right text-[10px] sm:text-xs"><colgroup><col className="w-[72px]"/><col className="w-[145px]"/><col className="w-[175px]"/><col className="w-[145px]"/><col className="w-[153px]"/></colgroup><thead><tr className="border-b border-slate-200 bg-slate-50 text-slate-500"><th className="p-2.5 font-black">الأسبوع</th><th className="p-2.5 font-black">التاريخ</th><th className="p-2.5 font-black">المساق / النشاط</th><th className="p-2.5 font-black">المستشفى</th><th className="p-2.5 font-black">المشرف</th></tr></thead><tbody>{displayedSchedule.map((item,index)=><tr key={`${item.block?.block_code}-${index}`} className={`border-b border-slate-100 last:border-0 ${item.item_type==='activity'?'bg-amber-50/70':'bg-white'}`}><td className="p-2.5 align-top"><span className={`inline-flex rounded-md px-2 py-1 font-black ${item.item_type==='activity'?'bg-amber-100 text-amber-800':'bg-teal-50 text-teal-800'}`}>{item.block?.from_week||'—'}</span></td><td className="p-2.5 align-top leading-5 text-slate-500"><span className="block">{formatDate(item.block?.start_date||null)}</span><span className="block text-[9px] text-slate-400">إلى {formatDate(item.block?.end_date||null)}</span></td><td className="p-2.5 align-top"><span className="block text-[9px] font-bold text-slate-400">{item.course?.code||'مساق سريري'}</span><span className={`mt-0.5 block font-black ${item.item_type==='activity'?'text-amber-800':'text-slate-800'}`}>{item.item_type==='activity'?(item.activity?.label||'نشاط أكاديمي'):(item.course?.name_ar||item.course?.name_en||'الدورة السريرية')}</span></td><td className="p-2.5 align-top font-bold text-slate-700">{item.item_type==='activity'?<span className="text-amber-700">لا يوجد دوام سريري</span>:(item.training_site?.name_ar||item.training_site?.name||'—')}</td><td className="p-2.5 align-top font-bold text-slate-700">{item.item_type==='activity'?<span className="text-amber-700">—</span>:(item.supervisor?.full_name_ar||item.supervisor?.name||'شاغر')}</td></tr>)}</tbody></table></div><p className="border-t border-slate-100 bg-slate-50 px-3 py-2 text-[9px] text-slate-400 sm:hidden">مرّر الجدول أفقيًا لرؤية جميع الأعمدة</p></div>}
