...
export type Student = { id:number; university_number:string; full_name_ar:string; full_name_en?:string|null; batch_year?:number|null; photo_url?:string|null };
...
export type AssessmentTemplate = { id:number; name_ar:string; name_en?:string|null; course_id?:number|null; batch_year?:number|null; version:number; is_active?:boolean; total_score:string|number; criteria:Criterion[] };
...
export type CriterionScore = { criterion_id:number; code?:string; name_ar?:string; name_en?:string|null; max_score:number; score?:number|null };
...
export const templateForGroup = (templates:AssessmentTemplate[],group:SupervisorGroup) => { const batch=group.batchYear??null; return templates.find(item=>item.course_id===group.courseId&&(item.batch_year??null)===batch) ?? templates.find(item=>item.course_id===group.courseId&&!item.batch_year) ?? templates.find(item=>!item.course_id&&(item.batch_year??null)===batch) ?? templates.find(item=>!item.course_id&&!item.batch_year); };
...
export function groupSupervisorAssignments(assignments:Assignment[]):SupervisorGroup[]{
  ...
  for(const item of assignments){
    const key=[item.distribution_version_id,item.rotation_block_id??'x',item.training_site_id??'x',item.student_subgroup_id??'x',item.student.batch_year??'x'].join('-');
    ...
    group.batchYear ??= item.student.batch_year ?? null;
