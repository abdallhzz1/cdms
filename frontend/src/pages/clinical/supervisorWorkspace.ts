...
export type Workspace = { supervisor:{full_name_ar:string;full_name_en:string}; assignments:Assignment[]; attendance_records:AttendanceRecord[]; assessments:Assessment[]; student_notes:SupervisorStudentNote[] };
export type SupervisorGroup = { key:string; assignmentId:number; rotationBlockId:number|null; subgroup:string; group:string; courseAr:string; courseEn:string; clinicalPeriodId:number|null;clinicalPeriodAr:string;clinicalPeriodEn:string;periodAr:string; periodEn:string; siteAr:string; siteEn:string; departmentAr:string; departmentEn:string; academicYear:string; startDate:string; endDate:string; students:Student[] };
...
export const groupName = (group:SupervisorGroup,ar:boolean) => `${ar?group.courseAr:group.courseEn} — ${ar?group.clinicalPeriodAr:group.clinicalPeriodEn} — ${group.group} (${group.subgroup})`;
