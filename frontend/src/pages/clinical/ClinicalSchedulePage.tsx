...
export function ClinicalSchedulePage() {
  ...
  return <div className="mx-auto max-w-[1400px] space-y-5 pb-12">
    ...
    {!items.length?<EmptyState title="لا توجد نتائج" message="لا توجد تعيينات منشورة تطابق الفلاتر. تأكد من توزيع مجموعة طلاب داخل خلية أسبوعية ثم نشر النسخة."/>:<>
      <div className="hidden flex-wrap gap-2 md:flex">{items.filter((item,index,list)=>item.supervisor&&list.findIndex(other=>other.supervisor?.id===item.supervisor?.id&&other.training_site?.id===item.training_site?.id)===index).map(item=><div key={`${item.supervisor?.id}-${item.training_site?.id}`} className="rounded-xl border border-teal-100 bg-teal-50 px-3 py-2 text-[10px] text-teal-900"><span className="font-black">{item.supervisor?.full_name_ar}</span><span className="mx-1 text-teal-500">·</span><span>{item.training_site?.name_ar}</span>{workDays(item)&&<span className="mt-1 block font-bold">أيام الدوام: {workDays(item)}</span>}</div>)}</div>
