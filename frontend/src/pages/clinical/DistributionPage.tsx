...
export function DistributionPage() {
  ...
  return <div className="mx-auto max-w-[1700px] space-y-5 pb-14" dir="rtl">
    ...
    {schedule?.rotation && schedule.version && <>
      <section className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><div><div className="flex items-center gap-2"><Grid3X3 className="h-5 w-5 text-teal-700" /><h2 className="font-black text-slate-800">{schedule.rotation.name}</h2><span className="rounded-full bg-slate-100 px-2 py-1 text-[10px] font-bold text-slate-600">{schedule.version.status}</span></div><p className="mt-1 text-xs text-slate-500">{schedule.blocks.length} أسبوع · {doctorsCount} طبيب · {schedule.rows.filter((row) => row.row_type === 'vacancy').length} شاغر · {schedule.subgroups.length} مجموعة فرعية</p></div><div className="flex flex-wrap gap-2">{can('distribution.schedule_rows.manage') && schedule.version.status !== 'published' && <Button variant="outline" onClick={() => openRow()}><Plus className="ml-1 h-4 w-4" />إضافة صف</Button>}{can('distribution.approve') && schedule.version.status !== 'published' && <Button variant="outline" onClick={() => approve.mutate()} isLoading={approve.isPending}><CheckCircle2 className="ml-1 h-4 w-4" />اعتماد</Button>}{can('distribution.publish') && schedule.version.status !== 'published' && <Button onClick={() => publish.mutate()} isLoading={publish.isPending}><Send className="ml-1 h-4 w-4" />نشر</Button>}</div></section>
    ...
    <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
      <button type="button" onClick={() => setHospitalsExpanded(!hospitalsExpanded)} className="flex w-full items-center justify-between p-4 text-right">
        <div className="flex items-center gap-2"><Building2 className="h-5 w-5 text-teal-700" /><div><h2 className="font-black text-slate-800">إدارة المستشفيات والأطباء</h2><p className="mt-0.5 text-[11px] text-slate-500">كل حساب يحمل دور «مشرف سريري» يظهر هنا كطبيب، ثم يُربط بالمستشفى المناسب.</p></div></div>
      ...
      {hospitalsExpanded && <div className="space-y-5 border-t border-slate-100 p-4">
        ...
        {unassignedDoctors.length > 0 && <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
          <div className="mb-3 flex items-center justify-between gap-2"><div><h3 className="text-sm font-black text-slate-800">أطباء بانتظار تعيين مستشفى</h3><p className="mt-0.5 text-[11px] text-slate-700">هؤلاء موجودون في المستخدمين بدور مشرف سريري ولن يظهروا في شبكة الجدول حتى يتم تعيين مستشفى.</p></div><span className="rounded-full bg-white px-2.5 py-1 text-xs font-black text-slate-800">{unassignedDoctors.length}</span></div>
          <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-3">{unassignedDoctors.map((doctor) => <div key={doctor.user_id} className="flex items-center justify-between gap-2 rounded-xl border border-slate-100 bg-white p-3"><div className="min-w-0"><p className="truncate text-xs font-black text-slate-800">{doctor.full_name_ar}</p><p dir="ltr" className="truncate text-left text-[10px] text-slate-500">{doctor.email}</p></div>{can('people.manage') && <Button variant="outline" size="sm" onClick={() => setAssigningDoctor({ doctor, siteId: '' })}>تعيين</Button>}</div>)}</div>
        </div>}
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{hospitals.map((hospital) => <article key={hospital.id} className="rounded-2xl border border-slate-200 p-4">
          <div className="flex items-start justify-between gap-2"><div><div className="flex items-center gap-2"><Building2 className="h-4 w-4 text-teal-700" /><h3 className="font-black text-slate-800">{hospital.name_ar}</h3></div><p className="mt-1 text-[10px] text-slate-500">{hospital.site_code}{hospital.city ? ` · ${hospital.city}` : ''} · {hospital.supervisors.length} طبيب</p></div>{can('training_sites.manage') && <button type="button" onClick={() => openHospital(hospital)} className="rounded-lg p-2 text-slate-500 hover:bg-slate-100" title="تعديل المستشفى"><Pencil className="h-4 w-4" /></button>}</div>
