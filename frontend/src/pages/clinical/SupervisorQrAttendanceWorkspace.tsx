...
export function SupervisorQrAttendanceWorkspace() {
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      try {
        ...
        const expectedPhase = selected.state === 'check_out_open' ? 'check_out' : 'check_in';
        if (payload.phase !== expectedPhase) throw new Error('QR phase mismatch');
  ...
  const visibleImage = imagePhase === selected?.state && expires > Date.now() ? image : '';
