...
export function ClinicalSupervisorsDirectoryPage() {
  ...
  <PageHeader title="دليل المشرفين والمواقع التدريبية" description="دليل تشغيلي موحّد لملفات المشرفين، تخصصاتهم، وربطهم بالمستشفيات التي تستخدمها شاشة التوزيع السريري.">
    ...
    {tab === 'doctors' ? <><section className="grid gap-3 rounded-3xl border border-slate-200 bg-white p-4 shadow-sm md:grid-cols-[1fr_15rem_auto]"><label className="flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3"><Search className="h-4 w-4 text-slate-400" /><input value={search} onChange={(event) => setSearch(event.target.value)} className="h-11 w-full bg-transparent text-xs font-bold outline-none" placeholder="بحث بالاسم أو البريد أو التخصص..." /></label><select value={hospitalFilter} onChange={(event) => setHospitalFilter(event.target.value)} className={inputClass}><option value="all">جميع المستشفيات</option><option value="unassigned">بدون مستشفى</option>{hospitals.map((hospital) => <option key={hospital.id} value={hospital.id}>{hospital.name_ar}</option>)}</select><div className="flex items-center justify-center rounded-xl bg-teal-50 px-4 text-xs font-bold text-teal-800">المعروضون: {filteredDoctors.length}</div></section>
      <section className="grid gap-3 md:hidden">{filteredDoctors.map((doctor) => { const profile = profiles.get(doctor.user_id); const doctorHospitals = hospitals.filter((hospital) => (doctor.training_site_ids ?? []).includes(hospital.id) || doctor.primary_site_id === hospital.id); return <article key={doctor.user_id} className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm"><button type="button" onClick={() => navigate(`/clinical-supervisors/${doctor.user_id}`)} className="flex w-full items-start gap-3 text-right"><div className="flex h-12 w-12 shrink-0 items-center justify-center overflow-hidden rounded-2xl border border-teal-100 bg-teal-50 text-sm font-black text-teal-700">{profile?.avatar_url ? <img src={profile.avatar_url} alt={doctor.full_name_ar} className="h-full w-full object-cover" /> : doctor.full_name_ar.slice(0, 1)}</div><div className="min-w-0 flex-1"><h2 className="truncate text-sm font-black text-slate-800">{doctor.full_name_ar}</h2><p dir="ltr" className="mt-1 truncate text-left text-[10px] text-slate-500">{doctor.email}</p><div className="mt-2 flex flex-wrap gap-1.5">{doctorHospitals.length ? doctorHospitals.map((hospital) => <span key={hospital.id} className="rounded-lg bg-teal-50 px-2 py-1 text-[10px] font-bold text-teal-700">{hospital.name_ar}</span>) : <span className="rounded-lg bg-slate-100 px-2 py-1 text-[10px] font-bold text-slate-600">بدون مستشفى</span>}<span className="rounded-lg bg-slate-50 px-2 py-1 text-[10px] font-bold text-slate-600">{doctor.specialty || profile?.specialty || 'التخصص غير محدد'}</span></div></div></button><div className="mt-3 flex flex-wrap gap-2 border-t border-slate-100 pt-3"><Button size="sm" onClick={() => navigate(`/clinical-supervisors/${doctor.user_id}`)}><ShieldCheck className="ml-1 h-4 w-4" />فتح الملف</Button>{can('clinical_supervisor_evaluations.create') && <Link to={`/clinical-supervisor-evaluations?supervisor=${doctor.user_id}`}><Button size="sm" variant="outline">تقييم رسمي</Button></Link>}{can('people.manage') && <Button size="sm" variant="outline" onClick={() => setAssigning({ doctor, siteId: doctor.primary_site_id ? String(doctor.primary_site_id) : '' })}><Building2 className="ml-1 h-4 w-4" />تعيين أو نقل</Button>}</div></article>; })}</section>

      <section className="hidden overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm md:block"><div className="overflow-x-auto"><table className="w-full min-w-[980px] text-right text-xs"><thead><tr className="border-b border-slate-200 bg-slate-50 text-slate-500"><th className="px-5 py-4">المشرف السريري</th><th className="px-5 py-4">المستشفى أو الموقع</th><th className="px-5 py-4">التخصص والدرجة</th><th className="px-5 py-4">بيانات التواصل</th><th className="px-5 py-4 text-center">الإجراء</th></tr></thead><tbody className="divide-y divide-slate-100">{filteredDoctors.map((doctor) => { const profile = profiles.get(doctor.user_id); const doctorHospitals = hospitals.filter((hospital) => (doctor.training_site_ids ?? []).includes(hospital.id) || doctor.primary_site_id === hospital.id); return <tr key={doctor.user_id} className="hover:bg-teal-50/40"><td className="px-5 py-4"><button type="button" onClick={() => navigate(`/clinical-supervisors/${doctor.user_id}`)} className="group flex items-center gap-3 text-right"><span className="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-xl border border-teal-100 bg-teal-50 font-black text-teal-700">{profile?.avatar_url ? <img src={profile.avatar_url} alt={doctor.full_name_ar} className="h-full w-full object-cover" /> : doctor.full_name_ar.slice(0, 1)}</span><span className="min-w-0"><span className="block font-black text-slate-800 transition group-hover:text-teal-700">{doctor.full_name_ar}</span>{doctor.full_name_en && <span dir="ltr" className="mt-1 block text-left text-[10px] text-slate-400">{doctor.full_name_en}</span>}</span></button></td><td className="px-5 py-4">{doctorHospitals.length ? <div className="flex flex-wrap gap-1.5">{doctorHospitals.map((hospital) => <span key={hospital.id} className="inline-flex items-center gap-1 rounded-lg bg-teal-50 px-2 py-1 font-bold text-teal-700"><Building2 className="h-3.5 w-3.5" />{hospital.name_ar}</span>)}</div> : <span className="rounded-lg bg-slate-100 px-2 py-1 font-bold text-slate-600">بدون مستشفى</span>}</td><td className="px-5 py-4"><p className="font-bold text-slate-700">{doctor.specialty || profile?.specialty || 'غير محدد'}</p><p className="mt-1 text-[10px] text-slate-500">{profile?.title || profile?.contract_type || 'الدرجة غير محددة'}</p></td><td className="px-5 py-4"><a href={`mailto:${doctor.email}`} dir="ltr" className="inline-flex items-center gap-1.5 text-left font-mono text-[10px] text-slate-500 hover:text-teal-700"><Mail className="h-3.5 w-3.5" />{doctor.email}</a></td><td className="px-5 py-4"><div className="flex flex-wrap justify-center gap-2"><Button size="sm" onClick={() => navigate(`/clinical-supervisors/${doctor.user_id}`)}><ShieldCheck className="ml-1 h-4 w-4" />فتح الملف</Button>{can('clinical_supervisor_evaluations.create') && <Link to={`/clinical-supervisor-evaluations?supervisor=${doctor.user_id}`}><Button size="sm" variant="outline">تقييم رسمي</Button></Link>}{can('people.manage') && <Button size="sm" variant="outline" onClick={() => setAssigning({ doctor, siteId: doctor.primary_site_id ? String(doctor.primary_site_id) : '' })}><Building2 className="ml-1 h-4 w-4" />تعيين أو نقل</Button>}</div></td></tr>; })}</tbody></table></div></section>
