...
export function PublicStudentRegistrationPage() {
  ...
  return <main dir="rtl" className="min-h-screen bg-[radial-gradient(circle_at_top,_#dff7f3_0,_#f8fafc_38%,_#f8fafc_100%)] px-3 py-4 text-slate-900 sm:px-6 sm:py-8">
    <div className="mx-auto max-w-2xl space-y-4 sm:space-y-5">
      <header className="overflow-hidden rounded-[28px] border border-white/80 bg-white/90 shadow-xl shadow-slate-200/50 backdrop-blur">
        ...
        <div className="p-5 text-center sm:p-7">
          ...
          {cycle&&<div className="mt-3 flex flex-wrap items-center justify-center gap-2 text-xs font-bold"><span className="rounded-full bg-slate-100 px-3 py-1.5">{levelName[cycle.academic_level]}</span><span className="rounded-full bg-blue-50 px-3 py-1.5 text-[#2f5a86]">{cycle.academic_year}</span><span className={`rounded-full px-3 py-1.5 ${cycle.status==='open'?'bg-emerald-50 text-emerald-700':'bg-amber-50 text-amber-700'}`}>{cycle.status==='open'?'التسجيل مفتوح':'التسجيل مغلق'}</span></div>}
        </div>
      </header>
      ...
      {cycle?.status==='closed'&&!options&&<Card className="rounded-[28px] p-8 text-center"><div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-amber-100 text-amber-700"><Lock/></div><h2 className="text-lg font-black">فترة التسجيل مغلقة حالياً</h2><p className="mt-2 text-sm leading-6 text-slate-500">للاستفسار أو معالجة حالة خاصة، يرجى التواصل مع إدارة الدائرة السريرية.</p></Card>}
