...
export function GradesPage() {
  ...
  const save = () => {
    ...
    const invalid = editableRows.some(row => { const item = drafts[row.student.id]; return item && ((item.osce !== '' && (+item.osce < 0 || +item.osce > 40)) || (item.written !== '' && (+item.written < 0 || +item.written > 40))); });
...
function GradeTable({ar,rows,drafts,setDrafts,totalFor,rowEditable,loading,error}:{ar:boolean;rows:RosterRow[];drafts:Record<number,Draft>;setDrafts:React.Dispatch<React.SetStateAction<Record<number,Draft>>>;totalFor:(row:RosterRow)=>string;rowEditable:(row:RosterRow)=>boolean;loading:boolean;error:unknown}) {
