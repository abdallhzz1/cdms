...
export function GradesPage() {
  ...
  const exportExcel = async () => {
    ...
    const header = sheet.addRow([ar?'الرقم الجامعي':'University ID', ar?'اسم الطالب':'Student name', `${componentLabel('clinical')} /${clinicalMax}`, `${componentLabel('osce')} /${osceMax}`, `${componentLabel('written')} /${writtenMax}`, `${ar?'المجموع':'Total'} /${planTotal}`, ar?'الحالة':'Status', ar?'ملاحظات':'Notes']);
  ...
  return <div className="space-y-5 pb-20">
    ...
    <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
      ...
      <GradeTable ar={ar} rows={filtered} drafts={drafts} setDrafts={setDrafts} totalFor={totalFor} rowEditable={rowEditable} loading={rosterQuery.isLoading} error={rosterQuery.error} clinicalLabel={`${componentLabel('clinical')} /${clinicalMax}`} osceLabel={`${componentLabel('osce')} /${osceMax}`} writtenLabel={`${componentLabel('written')} /${writtenMax}`} osceMax={osceMax} writtenMax={writtenMax}/>
