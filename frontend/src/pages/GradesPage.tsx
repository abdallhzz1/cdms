...
type AssessmentComponent = { code: 'clinical'|'osce'|'written'; name: string; weight: number|string; max_score: number|string };
type Course = { id: number; code: string; name_ar: string; name_en?: string; academic_level: Level; is_active?: boolean; assessment_components?: AssessmentComponent[] };
...
export function GradesPage() {
  ...
  const component = (code: AssessmentComponent['code']) => selectedCourse?.assessment_components?.find(item => item.code === code);
  const clinicalMax = Number(component('clinical')?.max_score ?? 20);
  const osceMax = Number(component('osce')?.max_score ?? 40);
  const writtenMax = Number(component('written')?.max_score ?? 40);
  const planTotal = Number(component('clinical')?.weight ?? 20) + Number(component('osce')?.weight ?? 40) + Number(component('written')?.weight ?? 40);
  const componentLabel = (code: AssessmentComponent['code']) => ({
    clinical: ar ? 'التقييم السريري' : 'Clinical assessment',
    osce: ar ? 'امتحان OSCE' : 'OSCE exam',
    written: ar ? 'الامتحان النظري' : 'Written exam',
  })[code];
  const displayedComponentLabel = (code: AssessmentComponent['code']) => component(code)?.name?.trim() || componentLabel(code);
  ...
  const rowEditable = (row: RosterRow) => can('grades.create') && !['submitted', 'approved'].includes(row.grade_entry?.status ?? 'draft');
  const editableRows = roster.filter(rowEditable);
  const hasEditableRows = editableRows.length > 0;
  const hasSubmittedRows = roster.some(row => row.grade_entry?.status === 'submitted');
  ...
  const mutation = useMutation({
    ...
    onSuccess: async (_, variables) => { setNotice({ ok: true, text: variables.path.includes('withdraw-approval') ? (ar ? 'تم سحب اعتمادك وإعادة الكشف لمساعد البحث والتدريس.' : 'Your approval was withdrawn and the sheet was returned to the research and teaching assistant.') : (ar ? 'تمت العملية بنجاح.' : 'Operation completed successfully.') }); setWithdrawReason(''); await Promise.all([queryClient.invalidateQueries({ queryKey: ['grade-roster', yearId, courseId] }), queryClient.invalidateQueries({ queryKey: ['grade-sheet-approval-status', yearId, courseId] })]); },
    onError: error => setNotice({ ok: false, text: errorText(error, ar ? 'تعذر إتمام العملية.' : 'Operation failed.') }),
  });
  const save = () => {
    if (!selectedCourse) return;
    const invalid = editableRows.some(row => { const item = drafts[row.student.id]; return item && ((item.osce !== '' && (+item.osce < 0 || +item.osce > osceMax)) || (item.written !== '' && (+item.written < 0 || +item.written > writtenMax))); });
    if (invalid) { setNotice({ ok: false, text: ar ? `علامة OSCE يجب ألا تتجاوز ${osceMax} والنظري ${writtenMax}.` : `OSCE must not exceed ${osceMax} and written must not exceed ${writtenMax}.` }); return; }
    mutation.mutate({ path: '/grade-entries/batch', body: { ...sheetKey, grades: editableRows.map(row => ({ student_id: row.student.id, max_score: 100, osce_score: drafts[row.student.id]?.osce === '' ? null : Number(drafts[row.student.id]?.osce), written_score: drafts[row.student.id]?.written === '' ? null : Number(drafts[row.student.id]?.written), notes: drafts[row.student.id]?.notes || null })) } });
  };
  ...
  const exportExcel = async () => {
    ...
    const header = sheet.addRow([ar?'الرقم الجامعي':'University ID', ar?'اسم الطالب':'Student name', `${displayedComponentLabel('clinical')} /${clinicalMax}`, `${displayedComponentLabel('osce')} /${osceMax}`, `${displayedComponentLabel('written')} /${writtenMax}`, `${ar?'المجموع':'Total'} /${planTotal}`, ar?'الحالة':'Status', ar?'ملاحظات':'Notes']);
  ...
  return <div className="space-y-5 pb-20">
    <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><h1 className="text-2xl font-black text-slate-900">{ar ? 'إدارة العلامات السريرية' : 'Clinical Grade Management'}</h1><p className="mt-1 text-sm text-slate-500">{ar ? 'كشف رسمي موحد: تقييم المشرف السريري + OSCE + الامتحان النظري.' : 'One official sheet: supervisor clinical assessment + OSCE + written exam.'}</p></div><button onClick={exportExcel} disabled={!roster.length} className="inline-flex items-center justify-center gap-2 rounded-2xl border border-teal-200 bg-white px-4 py-2.5 text-sm font-bold text-teal-800 disabled:opacity-40"><Download className="h-4 w-4" />{ar ? 'تصدير Excel' : 'Export Excel'}</button></header>
    ...
    {approvalStatus?.status==='pending'&&approvalStatus.acted_by_me&&<div className="rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-emerald-800"><div className="flex items-start gap-3"><CheckCircle2 className="mt-0.5 h-5 w-5 shrink-0"/><div><p className="text-sm font-black">{ar?'تم اعتماد مرحلة الكشف من قبلك':'You approved your stage of this sheet'}</p><p className="mt-1 text-xs font-bold">{ar?`لا يوجد إجراء مطلوب منك الآن. الكشف بانتظار: ${currentApprovalStep||'المرحلة التالية'}.`:`No further action is required from you. The sheet is awaiting: ${currentApprovalStep||'the next stage'}.`}</p></div></div>{can('grades.approve')&&<div className="mt-3 flex flex-col gap-2 border-t border-emerald-200 pt-3 sm:flex-row"><input value={withdrawReason} onChange={event=>setWithdrawReason(event.target.value)} placeholder={ar?'سبب سحب الاعتماد (إلزامي)...':'Reason for withdrawal (required)...'} className="min-w-0 flex-1 rounded-xl border border-emerald-200 bg-white px-3 py-2 text-xs text-slate-700 outline-none"/><button disabled={mutation.isPending} onClick={()=>withdrawReason.trim().length<3?setNotice({ok:false,text:ar?'اكتب سبب سحب الاعتماد بوضوح.':'Enter a clear withdrawal reason.'}):mutation.mutate({path:'/grade-entries/batch-withdraw-approval',body:{...sheetKey,reason:withdrawReason.trim()}})} className="rounded-xl border border-red-200 bg-white px-4 py-2 text-xs font-black text-red-700 disabled:opacity-40">{ar?'سحب اعتمادي وإعادة الكشف':'Withdraw my approval'}</button></div>}</div>}
    ...
    <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
      ...
      {selectedCourse&&<div className="flex flex-wrap items-center gap-2 border-b border-slate-100 bg-teal-50/50 px-4 py-2.5 text-[11px] font-bold text-teal-900"><span>{ar?'خطة التقييم:':'Assessment plan:'} {clinicalMax} + {osceMax} + {writtenMax} = {planTotal}</span><span className="text-teal-500">•</span><span>{ar?'السريري محسوب تلقائيًا من تقييمات المشرف':'Clinical is calculated automatically from supervisor assessments'}</span></div>}
      <GradeTable ar={ar} rows={filtered} drafts={drafts} setDrafts={setDrafts} totalFor={totalFor} rowEditable={rowEditable} loading={rosterQuery.isLoading} error={rosterQuery.error} clinicalLabel={`${displayedComponentLabel('clinical')} /${clinicalMax}`} osceLabel={`${displayedComponentLabel('osce')} /${osceMax}`} writtenLabel={`${displayedComponentLabel('written')} /${writtenMax}`} osceMax={osceMax} writtenMax={writtenMax}/>
...
function ScoreInput({value,disabled,max,onChange}:{value:string;disabled:boolean;max:number;onChange:(value:string)=>void}) { return <input type="number" min="0" max={max} step="0.25" value={value} disabled={disabled} onChange={event=>onChange(event.target.value)} className="mx-auto block w-24 rounded-xl border border-slate-200 px-2 py-2 text-center font-bold disabled:bg-slate-50 disabled:text-slate-500"/>; }

function GradeTable({ar,rows,drafts,setDrafts,totalFor,rowEditable,loading,error,clinicalLabel,osceLabel,writtenLabel,osceMax,writtenMax}:{ar:boolean;rows:RosterRow[];drafts:Record<number,Draft>;setDrafts:React.Dispatch<React.SetStateAction<Record<number,Draft>>>;totalFor:(row:RosterRow)=>string;rowEditable:(row:RosterRow)=>boolean;loading:boolean;error:unknown;clinicalLabel:string;osceLabel:string;writtenLabel:string;osceMax:number;writtenMax:number}) {
