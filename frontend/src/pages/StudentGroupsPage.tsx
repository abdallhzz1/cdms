...
export function StudentGroupsPage() {
  ...
  function saveSubgroup(event: FormEvent) {
    ...
    if (subgroupCapacity < editingSubgroup.current_students_count) {
      setError(`لا يمكن جعل السعة أقل من عدد الطلاب المسجلين حالياً (${editingSubgroup.current_students_count}).`);
      return;
    }
    run(() => apiFetch(`/group-registration-cycles/${selected.id}/subgroups/${editingSubgroup.id}`, {
      ...
      if (saved) {
        ...
        setNotice(`تم تحديث سعة المجموعة ${editingSubgroup.name} إلى ${subgroupCapacity} طلاب.`);
      }
    });
  }
  ...
  return <div dir="rtl" className="mx-auto max-w-[1500px] space-y-4 pb-14">
    ...
    <Modal isOpen={Boolean(editingSubgroup)} onClose={() => { setEditingSubgroup(null); setError(''); }} title="تعديل المجموعة الفرعية" maxWidth="md">
      {editingSubgroup && <form onSubmit={saveSubgroup} className="space-y-4">
        ...
        <label className="block"><span className="mb-1.5 block text-xs font-black text-slate-600">اسم المجموعة</span><input required value={editingSubgroupName} onChange={event => setEditingSubgroupName(event.target.value.toUpperCase())} className={inputClass} /></label>
        <label className="block"><span className="mb-1.5 block text-xs font-black text-slate-600">الحد الأقصى للطلاب</span><input required type="number" min={Math.max(1, editingSubgroup.current_students_count)} max={30} value={editingSubgroupCapacity} onChange={event => setEditingSubgroupCapacity(event.target.value)} className={inputClass} /><span className="mt-1.5 block text-[11px] text-slate-500">المسجلون حالياً: {editingSubgroup.current_students_count} طالب · السعة المسموحة من {Math.max(1, editingSubgroup.current_students_count)} إلى 30.</span></label>
        <div className="flex justify-end gap-2 border-t border-slate-100 pt-4"><Button type="button" variant="outline" onClick={() => { setEditingSubgroup(null); setError(''); }}>إلغاء</Button><Button type="submit" isLoading={busy}>حفظ السعة</Button></div>
      </form>}
    </Modal>
