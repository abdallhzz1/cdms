...
import { BookOpenCheck, CalendarDays, FileCheck2, Plus, ShieldCheck } from 'lucide-react';
...
export function StudentPoliciesPage() {
  const { can } = useAuth(); const queryClient = useQueryClient(); const [open, setOpen] = useState(false); const [error, setError] = useState('');
  const [form, setForm] = useState({ title_ar: 'مدونة سلوك طلبة الطب', title_en: 'Medical Students Code of Conduct', version_label: '', effective_date: '', academic_year_id: '', deadline: '', target_levels: ['fourth', 'fifth', 'sixth'] });
  ...
  return <div className="space-y-5" dir="rtl">
    <PageHeader title="سياسات وتعهدات الطلبة" description="نشر النسخة الرسمية، توثيق القراءة، ومتابعة النسخ الورقية الموقعة.">
      {can('student_policies.manage') && <button onClick={() => setOpen(true)} className="inline-flex items-center gap-2 rounded-xl bg-teal-700 px-4 py-2.5 text-sm font-bold text-white hover:bg-teal-800"><Plus className="h-4 w-4" />إنشاء حملة</button>}
    </PageHeader>
    ...
    <Modal isOpen={open} onClose={() => setOpen(false)} title="إنشاء حملة مدونة السلوك" maxWidth="2xl" backdropTone="light">
      <form className="space-y-4" onSubmit={e => { e.preventDefault(); create.mutate(); }}>
        <div className="rounded-xl bg-amber-50 p-3 text-xs font-medium text-amber-900">ارفع PDF النهائي المعتمد والمطبوع على ورق الكلية الرسمي. لا يقوم النظام بتعديل نص الوثيقة.</div>
        <div className="grid gap-3 sm:grid-cols-2"><label className="text-sm font-bold">العنوان العربي<input required value={form.title_ar} onChange={e=>setForm({...form,title_ar:e.target.value})} className="mt-1 w-full rounded-xl border p-3"/></label><label className="text-sm font-bold">العنوان الإنجليزي<input required value={form.title_en} onChange={e=>setForm({...form,title_en:e.target.value})} className="mt-1 w-full rounded-xl border p-3"/></label><label className="text-sm font-bold">رقم الإصدار<input required placeholder="مثال: 2026.1" value={form.version_label} onChange={e=>setForm({...form,version_label:e.target.value})} className="mt-1 w-full rounded-xl border p-3"/></label><label className="text-sm font-bold">تاريخ النفاذ<input required type="date" value={form.effective_date} onChange={e=>setForm({...form,effective_date:e.target.value})} className="mt-1 w-full rounded-xl border p-3"/></label><label className="text-sm font-bold">العام الأكاديمي<select required value={form.academic_year_id} onChange={e=>setForm({...form,academic_year_id:e.target.value})} className="mt-1 w-full rounded-xl border p-3"><option value="">اختر العام</option>{yearRows.map((y:any)=><option key={y.id} value={y.id}>{y.code}</option>)}</select></label><label className="text-sm font-bold">آخر موعد<input required type="date" value={form.deadline} onChange={e=>setForm({...form,deadline:e.target.value})} className="mt-1 w-full rounded-xl border p-3"/></label></div>
        <fieldset><legend className="mb-2 text-sm font-bold">السنوات المستهدفة</legend><div className="flex flex-wrap gap-2">{levels.map(l=><label key={l.value} className="flex items-center gap-2 rounded-xl border px-3 py-2 text-sm"><input type="checkbox" checked={form.target_levels.includes(l.value)} onChange={e=>setForm({...form,target_levels:e.target.checked?[...form.target_levels,l.value]:form.target_levels.filter(x=>x!==l.value)})}/>{l.label}</label>)}</div></fieldset>
        <label className="block text-sm font-bold">النسخة الرسمية PDF<input required accept="application/pdf" type="file" onChange={e=>setFile(e.target.files?.[0] ?? null)} className="mt-1 block w-full rounded-xl border p-3 text-sm"/></label>
