...
class StudentResource extends JsonResource
{
    public function toArray(Request $request): array
    {
        return [
            ...
            'academic_advisor' => $this->whenLoaded('academicAdvisor', fn () => $this->academicAdvisor ? [
                'id' => $this->academicAdvisor->id,
                'user_id' => $this->academicAdvisor->user_id,
            ...
            'current_group_name' => $this->whenLoaded('currentGroupAssignments', fn () => $this->currentGroupAssignments->first()?->group?->name
            ),
            'current_subgroup_name' => $this->whenLoaded('currentGroupAssignments', fn () => $this->currentGroupAssignments->first()?->subgroup?->name
            ),
            'registration_main_group' => $this->whenLoaded('groupRegistrationRosters', fn () => $this->groupRegistrationRosters->first()?->group?->name
            ),
            'registration_cycle_id' => $this->whenLoaded('groupRegistrationRosters', fn () => $this->groupRegistrationRosters->first()?->group_registration_cycle_id
            ),
        ];
    }
}
